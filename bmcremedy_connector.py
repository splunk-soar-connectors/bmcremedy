...
class BmcremedyConnector(BaseConnector):
    ...
    def _validate_integer(self, action_result, parameter, key, allow_zero=False):
        ...
        if parameter is not None:
            ...
            if not allow_zero and parameter == 0:
                return action_result.set_status(phantom.APP_ERROR, consts.BMCREMEDY_NON_NEG_NON_ZERO_INT_MESSAGE.format(param=key)), None
    ...
    def _parse_html_response(self, response):
        ...
        message = f"Status Code: {status_code}. Error Details: {error_text}"
    ...
    def _generate_api_token(self, action_result):
        ...
        # Make call
        response_status, response_dict, response = self._make_rest_call(
            consts.BMCREMEDY_TOKEN_ENDPOINT, action_result, headers=headers, data=payload
        )
    ...
    def _provide_attachment_details(self, attachment_list, action_result):
        ...
        if not attachment_list:
            ...
            return action_result.set_status(phantom.APP_ERROR, consts.BMCREMEDY_ERROR_INVALID_FIELDS.format(field="vault_id")), None, None
            ...
            for vault_id in attachment_list:
                ...
                for file_data in files_array:
                    if file_data[consts.BMCREMEDY_JSON_VAULT_ID] == vault_id:
                        ...
                        break
                if not file_found:
                    self.debug_print(f"{consts.BMCREMEDY_UNKNOWN_VAULT_ID}: {vault_id}")
                    return action_result.set_status(phantom.APP_ERROR, f"{consts.BMCREMEDY_UNKNOWN_VAULT_ID}: {vault_id}"), None, None
        except Exception as e:
    ...
    def _add_attachment(self, attachment_data, action_result):
        ...
        response_status, response_data = self._make_rest_call_abstract(
            consts.BMCREMEDY_COMMENT_ENDPOINT, action_result, data=data_params, method="post", files=files
        )
        if phantom.is_fail(response_status):
    ...
    def _get_url(self, action_result, incident_number):
        ...
        response_status, response_data = self._make_rest_call_abstract(consts.BMCREMEDY_GET_TICKET, action_result, params=params, method="get")
    ...
    def _make_rest_call_abstract(self, endpoint, action_result, data=None, params=None, method="post", accept_headers=None, files=None):
        ...
        # Make call
        rest_ret_code, response_data, response = self._make_rest_call(
            endpoint, intermediate_action_result, headers=headers, params=params, data=data, method=method, files=files
        )
    ...
    def _make_rest_call(self, endpoint, action_result, headers=None, params=None, data=None, method="post", files=None):
        ...
        try:
            ...
            error_message = f"{consts.BMCREMEDY_EXCEPTION_OCCURRED}. {self._get_error_message_from_exception(e)}"
        ...
        try:
            ...
            action_result_error_message = f"Proxy connection failed:  {error}"
            ...
            error_msg = "Error connecting to server. Connection refused from server for {}".format(f"{self._base_url}{endpoint}")
        ...
        if response.status_code in consts.ERROR_RESPONSE_DICT:
            self.debug_print(
                consts.BMCREMEDY_ERROR_FROM_SERVER.format(status=response.status_code, detail=consts.ERROR_RESPONSE_DICT[response.status_code])
            )
            ...
            if response_data and response_data.get("content"):
                try:
                    ...
                    if custom_error_message:
                        message_appended_text = f"{custom_error_message}{message_appended_text}"
                    response_message = f"Message Text: {message_text}. Message Appended Text: {message_appended_text}"
                except:
                    response_message = consts.BMCREMEDY_ERROR_JSON_PARSE.format(raw_text=response.text)
                    self.debug_print(response_message)

            # Set the action_result status to error, the handler function will most probably return as is
            action_result_error_msg = f"{consts.BMCREMEDY_ERROR_FROM_SERVER.format(status=response.status_code, detail=consts.ERROR_RESPONSE_DICT[response.status_code])}. {response_message}"
    ...
    def _create_ticket(self, param):
        ...
        if attachment_list:
            ...
            vault_details_status, add_attachment_details_param, attachment_data = self._provide_attachment_details(
                attachment_list, action_result
            )
        ...
        # Create incident using given input parameters
        response_status, response_data = self._make_rest_call_abstract(consts.BMCREMEDY_CREATE_TICKET, action_result, data=data, method="post")
        ...
        response_status, incident_response_data = self._make_rest_call_abstract(get_incident_data, action_result, method="get")
        ...
        try:
            ...
            error_message = self._get_error_message_from_exception(e)
            self.error_print(f"Error while summarizing data: {error_message}")
            return action_result.set_status(phantom.APP_ERROR, consts.BMCREMEDY_SUMMARY_ERROR.format(action_name="create_ticket"))
    ...
    def _update_ticket(self, param):
        ...
        if fields_param:
            ...
            response_status, response_data = self._make_rest_call_abstract(update_link, action_result, data=data, method="put")
    ...
    def _get_ticket(self, param):
        ...
        response_status, ticket_details = self._make_rest_call_abstract(
            consts.BMCREMEDY_GET_TICKET, action_result, params=action_params, method="get"
        )
        ...
        response_status, ticket_comment_details = self._make_rest_call_abstract(
            consts.BMCREMEDY_COMMENT_ENDPOINT, action_result, params=action_params, method="get"
        )

        if phantom.is_fail(response_status):
            self.debug_print(consts.BMCREMEDY_GET_COMMENT_ERROR.format(id=incident_id))
            return action_result.set_status(phantom.APP_ERROR, consts.BMCREMEDY_GET_COMMENT_ERROR.format(id=incident_id))
    ...
    def _list_tickets(self, param):
        ...
        # make rest call
        response_status, response_data = self._paginator(action_result, action_params, consts.BMCREMEDY_LIST_TICKETS, "entries", offset, limit)
    ...
    def _set_status(self, param):
        ...
        optional_parameter_list = ["assigned_support_company", "assigned_support_organization", "assigned_group", "assignee", "resolution"]
        ...
        response_status, response_data = self._make_rest_call_abstract(str(url), action_result, data=json.dumps(fields_param), method="put")
    ...
    def _add_comment(self, param):
        ...
        add_attachment_details_param.update({"Incident Number": incident_number, "Work Log Type": param[consts.BMCREMEDY_COMMENT_ACTIVITY_TYPE]})
